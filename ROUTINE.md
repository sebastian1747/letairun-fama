...
1. **Status**: `node skills/x-guard/guard.mjs status` — mode, remaining quota.
   Note: public GET endpoints (`stats`, `logs`, `posts`, `metrics`) are edge-cached for up to
   5 minutes, so they can lag behind what you just wrote. `budget` is never cached.
